...
class YoutubeAuditData(object):
    def __init__(self, load_state=False, root_path='./tmp', services=[youtube_monish, youtube_eric, youtube_ash]) -> None:
    ...
    def get_videos_and_comments(self, channel_id_csv) -> None:
        """TODO: add docstring"""
        ...
        channel_ids = pd.read_csv(channel_id_csv)['channel_ids'].to_list()
        while self.curr_channel_idx < len(channel_ids):  # for each channel
            ...
            while True:  # get all vids across iterations
                ...
                # for testing
                #print("Video next page token =", self.video_next_page_token)
                
                # Update video .csv contents
                ...
                while self.curr_video_idx < len(self.curr_video_data):  # for each video
                    ...
                    while True:  # get all comments across iterations
                        ...
                        # for testing
                        #print("Comment next page token =", self.comment_next_page_token)
    ...
    def get_videos_from_channel(self, channel_id) -> tuple:
        ...
        for _ in range(len(self.services)):  # cycle through api keys until successful
            try:
                res = channel2videos(self.services[self.service_idx],
                                     channel_id,
                                     self.video_next_page_token)
                break     
            except:
    ...
    def get_comments_from_video(self, video_id) -> tuple:
        ...
        for _ in range(len(self.services)):  # cycle through api keys until successful
            try:
                res = video2comments(self.services[self.service_idx],
                                     video_id,
                                     self.comment_next_page_token)
                break
            except:
