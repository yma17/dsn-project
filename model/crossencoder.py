...
_dataset = CrossEncoderDataset(iter=10000)
test_dataset = _dataset.test()

evaluator = CECorrelationEvaluator.from_input_examples(test_dataset, name='sts-dev')

cross_train_data_loader = DataLoader(_dataset, batch_size=batch_size)
cross_encoder = CrossEncoder(model_name, num_labels=1)
warmup_steps = math.ceil(len(cross_train_data_loader) * num_epochs * 0.1) 
cross_encoder.fit(train_dataloader=cross_train_data_loader, evaluator=evaluator, evaluation_steps=1000, epochs=num_epochs, warmup_steps=warmup_steps, output_path=cross_encoder_path)
